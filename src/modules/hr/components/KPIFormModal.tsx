...
export const KPIFormModal: FC<KPIFormModalProps> = ({ onClose, onSubmit }) => {
  const { register, handleSubmit, formState: { isSubmitting } } = useForm<Partial<KPI>>({
      ...
      <div className="w-full max-w-lg rounded-xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
          <div>
            ...
            <input required {...register('title')} className="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 focus:border-primary focus:outline-none" />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-300">Role</label>
            <input required {...register('role')} className="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 focus:border-primary focus:outline-none" />
          </div>

          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="mb-1 block text-sm font-medium text-slate-300">Current</label>
              <input required type="number" {...register('current', { valueAsNumber: true })} className="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 focus:border-primary focus:outline-none" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-slate-300">Target</label>
              <input required type="number" {...register('target', { valueAsNumber: true })} className="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 focus:border-primary focus:outline-none" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-slate-300">Unit</label>
              <input required {...register('unit')} className="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 focus:border-primary focus:outline-none" />
            </div>
          </div>
