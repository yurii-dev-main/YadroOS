...
export const OKRFormModal: FC<OKRFormModalProps> = ({ onClose, onSubmit }) => {
  const { register, handleSubmit, formState: { isSubmitting } } = useForm<Partial<OKR>>({
  ...
  const onFormSubmit = (data: Partial<OKR>) => {
    onSubmit({
      ...data,
      keyResults: [
        { id: '1', description: 'Sample Key Result', progress: 0 }
      ]
    });
    onClose();
  };
      ...
      <div className="w-full max-w-lg rounded-xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
          <div>
            ...
            <input required {...register('objective')} className="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 focus:border-primary focus:outline-none" />
          </div>
