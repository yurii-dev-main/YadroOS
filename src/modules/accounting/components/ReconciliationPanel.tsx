          ...
          pending.map((account) => (
            <div key={account.id} className="flex items-center justify-between rounded-md border border-slate-800 bg-slate-900/80 px-3 py-2">
              <div>
                ...
                <p className="text-xs text-slate-500">Current balance: {account.balance.toLocaleString('uk-UA', { style: 'currency', currency: account.currency })}</p>
              </div>
              <span className="text-xs uppercase tracking-wide text-amber-300">Pending</span>
            </div>
          ))
        ) : (
          <p className="text-xs text-slate-500">All accounts are reconciled with bank statements.</p>
        )}
      </CardContent>
    </Card>
  );
