...
export const RecommendationList = ({ recommendations, title }: RecommendationListProps) => (
  <div className="space-y-4">
    {title && <h3 className="text-sm font-semibold uppercase tracking-widest text-slate-400">{title}</h3>}
    <ul className="space-y-3">
      {recommendations.map((recommendation) => (
        <li key={recommendation.id} className="rounded-lg border border-slate-800 bg-slate-900/40 p-4">
